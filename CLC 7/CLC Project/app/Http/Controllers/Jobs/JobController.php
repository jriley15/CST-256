...
class JobController extends Controller
{
    ...
    function createJob(Request $request) {
        ...
        if ($userService->loggedIn($user)) {
            ...
            $job = new Job(-1, $jobTitle, $companyName, $jobDescription, $salary, $experienceRequired, $location, $user->getId(), now());
    ...
    function viewJob(Request $request) {
        ...
        if ($userService->loggedIn($user)) {
            ...
            $job = $jobService->getJob($request->input('ID'));
            
            return View("user.jobs.view-job", ['job' => $job, 'creator' => $userService->getUser($job->getOwnerID())]);
        } else {
    ...
    function editJob(Request $request) {
        ...
        if ($userService->loggedIn($user)) {
            ...
            if ($job->getOwnerID() == $user->getId() || $userService->isAdmin($user))
                return View("user.jobs.edit-job", ['job' => $job]);
    ...
    function updateJob(Request $request) {
        ...
        if ($userService->loggedIn($user)) {
            ...
            if ($jobService->updateJob($job) > 0) {
                ...
                return View("user.jobs.view-jobs", ['operation' => "No rows updated", 'jobs' => $jobService->getJobs()]);
            }
    ...
    //function that returns job search view
    function search(Request $request) {
        ...
        if ($userService->loggedIn($user)) {
            ...
            return View("user.jobs.view-job-search");
        } else {
    ...
    //function that returns jobs search view with jobs that match search criteria
    function doSearch(Request $request) {
        ...
        if ($userService->loggedIn($user)) {
            ...
            //validation rules
            $rules = ['search' => 'Required | Between:1,50'];
            
            
            $this->validate($request, $rules);
           
            $results = $jobService->searchJobs($search);
            
            if (count($results) > 0) {
                return View("user.jobs.view-job-search", ['jobResults' => $results, 'operation' => 'Results: '.count($results). " (100 max)"]);
            } else {
                return View("user.jobs.view-job-search", ['operation' => 'Results: None']);
            } 
